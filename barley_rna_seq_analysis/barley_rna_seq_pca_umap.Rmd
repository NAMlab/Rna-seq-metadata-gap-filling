...
# umap colored by tissue with custom colors
ggplot(umap_tissue_df, aes(UMAP1, UMAP2, color = tissue, shape = layout)) +
  geom_point(size = 1.1, alpha = 0.8) +
  theme_minimal() +
  labs(
...
# UMAP colored by treatment, with layout shapes
ggplot(umap_all_df %>% filter(!is.na(treatment) & !is.na(layout)),
       aes(UMAP1, UMAP2, color = treatment, shape = layout)) +
  geom_point(size = 1.1, alpha = 0.8) +
  theme_minimal() +
  labs(
    title = "UMAP of Barley RNA-seq (colored by treatment)",
...
```{r}
# check how layouts are distributed across treatments
table(meta_all$layout, meta_all$treatment)
...
```{r}
#check how samples are distributed across tissues and treatments
table(meta_all$tissue, meta_all$treatment)
...
```{r}
#check how samples are distributed across tissues ,treatments and layouts
table(meta_all$tissue, meta_all$treatment, meta_all$layout) 
...
# UMAP colored by developmental stage including NA values

ggplot(umap_all_df,
       aes(UMAP1, UMAP2, color = dev_stage, shape = layout)) +
  geom_point(size = 1.1, alpha = 0.8) +
  theme_minimal() +
  labs(
    title = "UMAP of Barley RNA-seq (colored by developmental stage, WITH NA)",
    color = "Developmental Stage",
    shape = "Layout"
  ) 
...
ggplot(
  umap_dev_df %>% 
    filter(!is.na(dev_stage) & !is.na(layout)),
  aes(UMAP1, UMAP2, color = dev_stage, shape = layout)
) +
  geom_point(size = 1.1, alpha = 0.8) +
  theme_minimal() +
...
# ggsave("umap_barley_developmental_stage.png", width = 7, height = 5, dpi = 300)
...
```{r}
# check counts of samples per developmental stage with NA
table(meta_all$developmental_stage, useNA = "ifany")
...
nrow(meta_dev)
nrow(meta_all %>% filter(!is.na(treatment)))
nrow(meta_tissue)
...
```{r}
# i want to save  some data files for future use which are tpm_matrix and tpm_df
saveRDS(tpm_matrix, file = "tpm_matrix_barley.rds")
saveRDS(tpm_df, file = "tpm_df_barley.rds") 
...
```{r}
# now i am running Anova  to quantify how much variance explained by each factor like tissue , treatment , developmental stage and sra study
# prepare metadata for ANOVA because anova needs factors 

meta_var <- meta_aligned %>%
  mutate(
    tissue    = factor(tissue),
    treatment = factor(treatment),
    dev_stage = factor(developmental_stage),
    sra_study = factor(`SRA Study`)
  )
...
```{r}
#combine metadata plus expression rows 
# put metadata (samples × variables)
expr <- as.data.frame(log_tpm_f)

# attach metadata columns
expr$sample_id <- meta_var$sample_id
expr$tissue    <- meta_var$tissue
expr$treatment <- meta_var$treatment
expr$dev_stage <- meta_var$dev_stage
expr$sra_study <- meta_var$sra_study
...
```{r}
#define anova function per gene
# this function will be applied to each gene (column) in the expression data frame
expression ~ tissue + treatment + dev_stage + sra_study
...
get_variance_components <- function(y, meta) {
  
  fit <- aov(y ~ tissue + treatment + dev_stage + sra_study, data = meta)
  
  ss <- summary(fit)[[1]][, "Sum Sq"]
  total <- sum(ss)
  
  prop <- ss / total
  names(prop) <- c("tissue", "treatment", "dev_stage", "sra_study", "residual")
...
```{r}
# apply anova function to each gene
var_results <- apply(log_tpm_f, 2, get_variance_components, meta = meta_var)
...
```{r}
# convert results to data frame for easier handling
var_df <- as.data.frame(t(var_results))
...
```{r}
# how many NAs per column? (just to check)
colSums(is.na(var_df))

# overall fraction of variance explained by each factor
overall_var <- colMeans(var_df, na.rm = TRUE)
overall_var
...
overall_percent <- overall_var * 100
overall_percent
...
# overall_percent is a named vector
overall_percent

#   tissue treatment dev_stage sra_study residual
#  24.87     9.33      0.37      31.84    33.59   (approx)

# Put into a data frame for ggplot
var_summary <- data.frame(
  factor  = names(overall_percent),
  percent = as.numeric(overall_percent)
)

# Order bars from largest to smallest
var_summary$factor <- factor(
  var_summary$factor,
  levels = var_summary$factor[order(var_summary$percent, decreasing = TRUE)]
)


# Barplot
ggplot(var_summary, aes(x = factor, y = percent, fill = factor)) +
  geom_col(width = 0.7) +
  ...
  xlab("Factor") +
  ggtitle("Fraction of gene expression variance explained by each factor") +
...
# save the variance explained barplot
#ggsave("anova_variance_explained_barley.png", width = 7, height = 5, dpi = 300)
...
# performing post-hoc test to see which group drive that varience in each factor like tissue , treatment , developmental stage and sra study
...
pca_df <- data.frame(
  ...
  tissue = meta_aligned$tissue,
  treatment = meta_aligned$treatment,
  dev_stage = meta_aligned$developmental_stage,
  sra_study = meta_aligned$`SRA Study`
)
...
anova_tissue <- aov(PC1 ~ tissue, data = pca_df)
...
#anova for treatment
anova_treat <- aov(PC1 ~ treatment, data = pca_df)
summary(anova_treat)
...
```{r}
#anova for developmental stage
anova_dev <- aov(PC1 ~ dev_stage, data = pca_df)
summary(anova_dev)
...
```{r}
#anova for sra study
anova_sra <- aov(PC1 ~ sra_study, data = pca_df)
summary(anova_sra)
...
```{r}
#post-hoc test for tissue

tukey_tissue <- TukeyHSD(anova_tissue)
tukey_tissue
...
tukey_treat <- TukeyHSD(anova_treat)
tukey_treat
...
```{r}
#post-hoc test for developmental stage
tukey_dev <- TukeyHSD(anova_dev)
tukey_dev
...
```{r}
#post-hoc test for sra study
tukey_sra <- TukeyHSD(anova_sra)
tukey_sra
